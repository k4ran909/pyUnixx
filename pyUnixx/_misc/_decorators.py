...
from ..version import unixx_version as ult_ver
...
def unixx_cmd(
    pattern=None, manager=False, unixx_bot=unixx_bot, asst=asst, **kwargs
):
    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            try:
                ...
                await unixx_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await unixx_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        ...
                        Button.url(
                            "Repl",
                            "https://replit.com/@TeamUnixx/UnixxStringSession",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = (
                    "**Unixx Client Error:** `Forward this to` @UnixxSupport\n\n"
                )
                ftext += "**Py-Unixx Version:** `" + str(pyver)
                ftext += "`\n**Unixx Version:** `" + str(ult_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START UNIXX CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Unixx Client Error:** `Forward this to` @UnixxSupport\n\n",
                        )
                else:

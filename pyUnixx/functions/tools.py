...
# Thanks u for ChatBotApi
async def get_chatbot_reply(message):
    chatbot_base = "https://kukiapi.xyz/api/apikey=UNIXXUSERBOT/Unixx/{}/message={}"
    req_link = chatbot_base.format(
        unixx_bot.me.first_name or "unixx user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    gd_name = unixx_bot.full_name
    short_name = gd_name[:30]
    profile_url = (
        f"https://t.me/{unixx_bot.me.username}" if unixx_bot.me.username else None
    )
    try:
        ...
        if "SHORT_NAME_TOO_LONG" in str(er):
            TelegraphClient.create_account(
                short_name="unixxuser", author_name=gd_name, author_url=profile_url
            )
        else:
            LOGS.exception(er)
            return
...
async def Carbon(
    ...
    **kwargs,
    ...
    if rayso:
        ...
        kwargs["title"] = kwargs.get("title", "Unixx")
    else:
        kwargs["code"] = code

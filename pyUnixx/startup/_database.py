...
class MongoDB:
    def __init__(self, key, dbname="UnixxDB"):
        self.dB = MongoClient(key, serverSelectionTimeoutMS=5000)
        self.db = self.dB[dbname]
        self.re_cache()

    def __repr__(self):
        return f"<Unixx.MonGoDB\n -total_keys: {len(self.keys())}\n>"
...
# for SQL Implementation in Unixx.
#
...
class SqlDB:
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS Unixx (unixxCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('Unixx')) AS size"
        )
    ...
    def keys(self):
        self._cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name  = 'unixx'"
        )  # case sensitive
        data = self._cursor.fetchall()
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM Unixx")
    ...
    def set_key(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE Unixx DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE Unixx ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO Unixx ({key}) values (%s)", (str(value),))
        return True
    ...
    def del_key(self, key):
        if key in self._cache:
            del self._cache[key]
        try:
            self._cursor.execute(f"ALTER TABLE Unixx DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE Unixx")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS Unixx (unixxCli varchar(70))"
        )
        return True

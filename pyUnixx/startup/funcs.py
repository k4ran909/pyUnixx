...
def startup_stuff():
    ...
    if CT:
        urlretrieve(CT, "resources/extras/unixx.jpg")
    ...
    if TZ and timezone:
        try:
            ...
            LOGS.critical(
                "Incorrect Timezone ,\nCheck Available Timezone From Here https://telegra.ph/Unixx-08-28\nSo Time is Default UTC"
            )
            os.environ["TZ"] = "UTC"
            time.tzset()
...
async def autobot():
    from .. import udB, unixx_bot

    if udB.get_key("BOT_TOKEN"):
        return
    await unixx_bot.start()
    ...
    if who.username:
        ...
        username = "unixx_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    await unixx_bot(UnblockRequest(bf))
    await unixx_bot.send_message(bf, "/cancel")
    await asyncio.sleep(1)
    await unixx_bot.send_message(bf, "/newbot")
    await asyncio.sleep(1)
    isdone = (await unixx_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("That I cannot do.") or "20 bots" in isdone:
        ...
        sys.exit(1)
    await unixx_bot.send_message(bf, name)
    await asyncio.sleep(1)
    isdone = (await unixx_bot.get_messages(bf, limit=1))[0].text
    if not isdone.startswith("Good."):
        await unixx_bot.send_message(bf, "My Assistant Bot")
        await asyncio.sleep(1)
        isdone = (await unixx_bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.critical(
            ...
            sys.exit(1)
    await unixx_bot.send_message(bf, username)
    await asyncio.sleep(1)
    isdone = (await unixx_bot.get_messages(bf, limit=1))[0].text
    await unixx_bot.send_read_acknowledge("botfather")
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "unixx_" + (str(who.id))[6:] + str(ran) + "_bot"
        await unixx_bot.send_message(bf, username)
        await asyncio.sleep(1)
        nowdone = (await unixx_bot.get_messages(bf, limit=1))[0].text
        if nowdone.startswith("Done!"):
            token = nowdone.split("`")[1]
            udB.set_key("BOT_TOKEN", token)
            await enable_inline(unixx_bot, username)
            LOGS.info(
...
async def autopilot():
    from .. import asst, udB, unixx_bot

    channel = udB.get_key("LOG_CHANNEL")
    new_channel = None
    if channel:
        try:
            chat = await unixx_bot.get_entity(channel)
        except BaseException:
    ...
    assistant = True
    try:
        await unixx_bot.get_permissions(int(channel), asst.me.username)
    except UserNotParticipantError:
        try:
            await unixx_bot(InviteToChannelRequest(int(channel), [asst.me.username]))
        except BaseException as er:
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://te.legra.ph/file/c0f39bffadfd41066b1ac.png", "channelphoto.jpg"
        )
        ll = await unixx_bot.upload_file(photo)
        try:
            await unixx_bot(
...
async def customize():
    ...
    try:
        ...
        if not unixx_bot.me.username:
            sir = unixx_bot.me.first_name
        else:
            sir = f"@{unixx_bot.me.username}"
        file = random.choice(
            [
                "https://te.legra.ph/file/3eeca9731b55af4c68ad3.png",
                "https://te.legra.ph/file/3eeca9731b55af4c68ad3.png",
                "resources/extras/unixx_assistant.jpg",
            ]
        )
        ...
        await asyncio.sleep(1)
        await unixx_bot.send_message("botfather", "/cancel")
        await asyncio.sleep(1)
        await unixx_bot.send_message("botfather", "/setuserpic")
        await asyncio.sleep(1)
        isdone = (await unixx_bot.get_messages("botfather", limit=1))[0].text
        if isdone.startswith("Invalid bot"):
            LOGS.info("Error while trying to customise assistant, skipping...")
            return
        await unixx_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await unixx_bot.send_file("botfather", file)
        await asyncio.sleep(2)
        await unixx_bot.send_message("botfather", "/setabouttext")
        await asyncio.sleep(1)
        await unixx_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await unixx_bot.send_message(
            "botfather", f"✨ Hello ✨!! I'm Assistant Bot of {sir}"
        )
        await asyncio.sleep(2)
        await unixx_bot.send_message("botfather", "/setdescription")
        await asyncio.sleep(1)
        await unixx_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await unixx_bot.send_message(
            "botfather",
            f"✨ Powerful Unixx Assistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @TeamUnixx ✨",
        )
        await asyncio.sleep(2)
...
async def plug(plugin_channels):
    ...
    for chat in plugin_channels:
        LOGS.info(f"{'•'*4} {chat}")
        try:
            async for x in unixx_bot.iter_messages(
                ...
                try:
                    ...
                    LOGS.info(f"Unixx - PLUGIN_CHANNEL - ERROR - {plugin}")
                    LOGS.exception(e)
                    os.remove(plugin)
...
async def ready():
    ...
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """🎇 **Thanks for Deploying Unixx Userbot!**
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "https://telegra.ph/file/525b1928564d03acd07f8.png"
        BTTS = Button.inline("• Click to Start •", "initft_2")
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = f"**Unixx has been deployed!**\n➖➖➖➖➖➖➖➖➖➖\n**UserMode**: {inline_mention(unixx_bot.me)}\n**Assistant**: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖\n**Support**: @TeamUnixx\n➖➖➖➖➖➖➖➖➖➖"
        BTTS, PHOTO = None, None
        prev_spam = udB.get_key("LAST_UPDATE_LOG_SPAM")
        if prev_spam:
            try:
                await unixx_bot.delete_messages(chat_id, int(prev_spam))
            except Exception as E:
    ...
    try:
        ...
        try:
            await (await unixx_bot.send_message(chat_id, str(e))).delete()
            spam_sent = await asst.send_message(chat_id, MSG, file=PHOTO, buttons=BTTS)
        ...
        LOGS.info(el)
        try:
            spam_sent = await unixx_bot.send_message(chat_id, MSG)
    ...
    try:
        ...
        await unixx_bot(JoinChannelRequest("@TheUnixx"))
    except BotMethodInvalidError:
        pass
    except ChannelsTooMuchError:
        LOGS.info("Join @TheUnixx to know about new Updates...")
    except ChannelPrivateError:
        LOGS.critical(
            "You are Banned from @TheUnixx for some reason. Contact any dev if you think there is some mistake..."
        )
        import sys
...
async def WasItRestart(udb):
    ...
    if not key:
        return
    from .. import asst, unixx_bot

    try:
        data = key.split("_")
        who = asst if data[0] == "bot" else unixx_bot
...
async def enable_inline(unixx_bot, username):
    bf = "BotFather"
    await unixx_bot.send_message(bf, "/setinline")
    await asyncio.sleep(1)
    await unixx_bot.send_message(bf, f"@{username}")
    await asyncio.sleep(1)
    await unixx_bot.send_message(bf, "Search")
    await unixx_bot.send_read_acknowledge(bf)

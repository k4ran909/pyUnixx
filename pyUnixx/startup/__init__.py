...
if run_as_module:
    ...
    from ..version import __version__ as __pyUnixx__
    from ..version import unixx_version

    file = f"unixx{sys.argv[6]}.log" if len(sys.argv) > 6 else "unixx.log"

    if os.path.exists(file):
        os.remove(file)

    HOSTED_ON = where_hosted()
    LOGS = getLogger("pyUniLogs")
    ...
    LOGS.info(f"py-Unixx Version - {__pyUnixx__}")
    LOGS.info(f"Telethon Version - {__version__} [Layer: {LAYER}]")
    LOGS.info(f"Unixx Version - {unixx_version} [{HOSTED_ON}]")
